...
def _extract_box_score(soup_bs, team=None):
...
def make_path(date, basedir='./'):
    path = date2path(date, basedir=basedir).replace('//','/')
    try:
        os.makedirs(path)
    except OSError:
        pass

def get_fname(date, home_team, away_team, basedir='./'):
